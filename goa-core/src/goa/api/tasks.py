...
from goa.errors import InvalidEventShape, NotAParticipant, TaskNotFound
...
@router.post("/tasks/upsert", response_model=UpsertTaskResponse)
...
@router.post("/tasks/{task_id}/close", response_model=CreateTaskResponse)
...
@router.post("/tasks/{task_id}/events", status_code=201, response_model=AppendEventResponse)
...
@router.get("/pending", response_model=list[PendingRow], response_model_by_alias=True)
...
@router.get("/tasks/{task_id}/children", response_model=ListChildrenResponse)
