...
async def test_purge_owner_releases_all_and_is_scoped(store: MemoryStore) -> None:
    owner, other = uuid4(), uuid4()
    for k in ("a", "b", "c"):
        await _put(store, _entry(owner, k, 1))
    await _put(store, _entry(other, "a", 1))
    # Releases every entry the owner has (the participant-deletion wipe path)…
    assert await store.purge_owner(owner) == 3
    assert await store.list_memory(owner) == []
    # …without touching another owner's memory, and is idempotent.
    assert await store.purge_owner(owner) == 0
    assert len(await store.list_memory(other)) == 1
