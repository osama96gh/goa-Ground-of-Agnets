...
from goa.errors import AUTH_401, VALIDATION_422
...
@router.get(
    "/stream",
    summary="Open the event stream (SSE)",
    responses={
        200: {
            "description": "A long-lived Server-Sent Events stream (text/event-stream) of `event`, `ping`, and `stream.gap` frames. Reconnect with the `Last-Event-ID` header to replay missed events."
        },
        **AUTH_401,
        **VALIDATION_422,
    },
