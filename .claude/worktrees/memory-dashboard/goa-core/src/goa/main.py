...
from goa.api.memory import router as memory_router
...
_API_DESCRIPTION = """\
Goa is a hub for **multi-party agent coordination** with persistent task state.

Participants (agents and services) register once, hold a single long-lived SSE
connection, and exchange typed **events** inside named **tasks**. Sub-tasks give
first-class private delegation, and each participant has **agent-private memory**
that persists across tasks and sessions.

**Auth.** All endpoints except `POST /participants` require a bearer token:
`Authorization: Bearer <api_key>`. Admin endpoints use the deployment's admin
token instead.

**Errors.** Every non-2xx response uses one envelope —
`{"error": {"code": "...", "message": "..."}}`. Switch on the stable `code`;
`message` is human-readable and may change.
"""

# Tag metadata — the grouped sections rendered in /docs and /redoc.
_OPENAPI_TAGS = [
    {"name": "Participants", "description": "Register and discover agents and services."},
    {"name": "Tasks", "description": "Create tasks, append events, and read task state and pending questions."},
    {"name": "Blobs", "description": "Upload and download attachment bytes bound to a task."},
    {"name": "Stream", "description": "Long-lived SSE connection delivering inbound events."},
    {"name": "Memory", "description": "Agent-private, cross-task key→document memory owned by the caller."},
    {"name": "Admin", "description": "Admin-token-gated read/firehose endpoints (present only when an admin token is configured)."},
]
...
def create_app(
    ...
    *,
    ...
    ADK-style entry point — consumers ship a `Persistence` whose four
    fields satisfy the `TaskLog`, `ParticipantStore`, `BlobStore`, and
    `MemoryStore` Protocols. The bundle is the single injection point:
    callers must provide all four backends together, so a half-wired
    deployment (one Protocol persistent, the others silently in-memory)
    is structurally impossible. See `goa.repos.protocols` for the
    contracts and `goa.repos.memory` for reference impls.

    >>> from goa import create_app, Persistence
    >>> app = create_app()  # zero-config in-memory
    >>>
    >>> # Or with custom backends:
    >>> from my_pg import PostgresTaskLog, PostgresParticipantStore, PostgresMemoryStore
    ...
    app = FastAPI(
        title="Goa",
        version="0.5.0",
        summary="Ground of Agents — a hub for multi-party agent coordination.",
