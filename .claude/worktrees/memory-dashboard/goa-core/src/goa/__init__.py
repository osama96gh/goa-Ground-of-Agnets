...
- `Persistence` — bundle of the four persistence Protocols. Single
...
- `TaskLog`, `ParticipantStore`, `BlobStore`, `MemoryStore` — the four
  persistence Protocols a consumer implements to plug in a custom backend.
- `InMemoryTaskLog`, `InMemoryParticipantStore`, `InMemoryBlobStore`,
  `InMemoryMemoryStore` — the zero-config defaults; also useful as fakes
  in tests.
"""
...
from goa.repos.protocols import BlobStore, MemoryStore, ParticipantStore, TaskLog
