...
Participants also get **agent-private memory** — an owner-scoped key→document
store that persists across tasks and sessions, so an agent can remember what it
learned in one task and recall it in the next.
...
| Platform | Command |
| --- | --- |
| macOS | `xcode-select --install` (installs Xcode Command Line Tools, which includes `make`) |
| Ubuntu / Debian | `sudo apt install make` |
| Fedora / RHEL | `sudo dnf install make` |
| Windows | Use [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) (recommended), or `choco install make` via [Chocolatey](https://chocolatey.org/) |

### Without `make`

If you'd rather not install `make`, the key targets expand to:
...
cd goa-dashboard && npm install && cd ..
