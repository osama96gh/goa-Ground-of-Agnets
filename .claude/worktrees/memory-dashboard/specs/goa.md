...
Goa owns: task creation and membership, event delivery, pending-question state, agent-private memory (§9), authentication, and observability.
...
| **Memory**         | A persistent, **agent-private** `(key, value, tags)` entry owned by exactly one participant. Lives outside any task and spans tasks and sessions. Read and written only by its owner, so it never crosses the task-boundary seal (§7). See §9. |
...
Where the event log records *what happened in a task*, **memory** records *what an agent knows* — facts it wants to carry across tasks and sessions: a customer's stated preference, a resolution it reached, a policy fragment it derived. Memory is the one piece of durable participant state that lives **outside** any task.

- **Owner-scoped, always.** Every entry has exactly one `owner_id` — the participant that wrote it — and that is the *sole* authorization key. A participant reads and writes only its own memory; there is no cross-participant read. Because an agent only ever stores what it already observed, recall can never widen what a participant can see — so memory sits cleanly alongside the task-boundary model (§7): it spans tasks, but never the seal. Cross-participant **shared** memory would be a deliberate hole in that seal and is out of scope here (it belongs with per-participant ACLs; see the roadmap).
- **Operator read, never write.** Owner-scoping is the *participant's* sole key — but the deployment operator, holding the out-of-band `GOA_ADMIN_TOKEN` (the same secret behind the `/admin` event firehose), can *read* any participant's memory for observability and moderation. The admin token never writes or forgets: a participant's memory must reflect only what it observed, so there is no admin-as-agent write. Operator-side *forget* (moderation / GDPR erasure) is deferred — see the roadmap.
- **Keyed documents, agent-namespaced.** An entry is a `(key, value, tags)` triple; `value` is any JSON value. Keys are opaque to Goa — agents namespace them however they like (`user:U123:tone`, `policy/refund/v2`). That convention is how "per-user" or "per-topic" memory is expressed without Goa modeling those concepts. Recall is **exact**: by key, by key-prefix, or by AND-ed tags — no embeddings or semantic search in this release.
- **Pull, not push.** Goa never injects memory into a task or an inference call — it does not run the agent's model. The agent queries its memory and decides what to use; the agent decides what to store. (This is the same reason there is no server-side summarization: it would make Goa LLM-aware.)
- **Overwrite, no history.** Writing an existing key replaces its `value` and `tags`; the original `created_at` is preserved and `updated_at` advances. There is no version log and no TTL/expiry in this release.
- **Forget by prefix.** Deletion is per-key or by key-prefix — the latter is the "forget everything about X" path (prefix matching is an exact byte-range scan, *not* a SQL `LIKE` pattern, so `_`/`%` in keys are literal). A participant's entire memory is released when the participant is deleted.
