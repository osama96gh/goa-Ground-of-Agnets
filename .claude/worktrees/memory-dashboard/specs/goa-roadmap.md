...
| Admin-side memory forget | Operator moderation / GDPR erasure of a participant's memory (delete-by-key, forget-by-prefix) via the admin token. Read-only admin access ships today (`GET /admin/participants/{id}/memory`); deletion is intentionally not exposed — today only the owning participant can forget, or delete the participant to release all of its memory. |
